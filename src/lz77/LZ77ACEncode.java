...
public class LZ77ACEncode {

	public static void main(String [] args) throws IOException {
		String input_file_name = "data/uncompressed.txt";
		String output_file_name = "data/lz77-ac-compressed.dat";
		 ...
		 * Bytes encoded as unmatched use 256 value adaptive integer frequency model.
		 * Search offset and match length use adaptive integer frequency model sized as needed.
		 */
		...
		while (num_symbols_encoded < num_symbols) {
			...
			if (longest_match_offset == -1) {
				...
				ac.encode(longest_match_offset-1, match_offset_model, bit_sink);
				match_offset_model.addToCount(longest_match_offset-1);
				
				ac.encode(longest_match_length-1, match_length_model, bit_sink);
				match_length_model.addToCount(longest_match_length-1);
